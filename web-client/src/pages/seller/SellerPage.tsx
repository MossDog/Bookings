...
import { getProfileFromSlug, getServicesFromId } from '@/utils/sellerProfileUtils';

export default function SellerPage() {
  const { slug } = useParams<{ slug: string }>();  const [seller, setSeller] = useState<Seller | undefined>();
  ...
  useEffect(() => {
    const fetchSeller = async () => {
      ...
      setSeller(data);      const servs = await getServicesFromId(data.user_id);
      setServices(servs || []);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col lg:flex-row max-w-[1440px] mx-auto px-4 md:px-10 gap-6 mt-8">
        {/* Left Side: Main Content */}        {seller && (
          <div className="flex-1 max-w-4xl">
            <HighlightWidget userId={seller.user_id} />
