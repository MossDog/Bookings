...
export default function SellerOpeningHours({
  ...
  const handleAddBreak = (dayId: string) => {
    setSchedule(prev => ({
      ...
      [dayId]: {
        ...prev[dayId],
        breaks: [...prev[dayId].breaks, { startTime: '12:00 PM', endTime: '1:00 PM' }]
      }
    }));
  };
  ...
  const handleBreakTimeChange = (dayId: string, breakIndex: number, field: 'startTime' | 'endTime', value: string) => {
    setSchedule(prev => ({
      ...prev,
      [dayId]: {
        ...prev[dayId],
  ...
  return (
    <div className="card bg-base-100 shadow-md h-full">
      <div className="bg-base-200/50 p-6 border-b border-base-200">
        <h3 className="text-xl font-semibold text-base-content">Opening Hours</h3>
      ...
      <div className="card-body p-6 h-full overflow-hidden">
        <div className="flex overflow-x-auto h-full gap-4 pb-4 -mx-2 px-2 snap-x">
          {days.map(day => (
            <div key={day.id} className="h-full card bg-base-200 w-[300px] flex-shrink-0 snap-start">
                ...
                {schedule[day.id].isClosed ? (
                  <div className="flex flex-col items-center justify-center py-8 text-base-content/70">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mb-3 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10 2a8 8 0 018 8c0 4.418-3.582 8-8 8a8 8 0 01-8-8 8 8 0 018-8zM10 6v4l3 3" />
                    </svg>
                    <p className="text-sm font-medium mb-1">Closed on {day.label}s</p>
                    <p className="text-xs">No appointments available on this day</p>
                  </div>
                ) : (
                  <>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="form-control">
                            <select 
                              ...
                              onChange={(e) => handleBreakTimeChange(day.id, index, 'startTime', e.target.value)}
                            >
                              {timeOptions.map(time => (
                                <option key={time} value={time}>{time}</option>
                              ))}
                          ...
                          <div className="form-control">
                            <select 
                              ...
                              onChange={(e) => handleBreakTimeChange(day.id, index, 'endTime', e.target.value)}
                            >
                              {timeOptions.map(time => (
                                <option key={time} value={time}>{time}</option>
                              ))}
