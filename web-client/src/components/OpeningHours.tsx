...
export default function OpeningHoursWidget({ sellerId, timezone }: Props) {
  const [workingHours, setWorkingHours] = useState<Record<
    number,
          ...
          <ul className="menu menu-xs bg-base-100 w-full p-0">
            {daysOfWeek.map((day, idx) => {
              ...
              return (
                <li key={idx}>
                  <div
                    className={`flex justify-between py-2 ${isToday ? "bg-base-200 rounded-lg font-semibold" : ""}`}
                  >
                    <span className="text-base-content">{day}</span>
                    {hours ? (
                      <span className="text-primary">
                        {DateTime.fromFormat(hours.start, "HH:mm:ss", {
                          zone: timezone,
                        }).toFormat("hh:mm a")}{" "}
                        –{" "}
                        {DateTime.fromFormat(hours.end, "HH:mm:ss", {
                          zone: timezone,
                        }).toFormat("hh:mm a")}
                      </span>
                    ) : (
