...
export const fileExistsInBucket = async (bucketName: string, filePath: string): Promise<boolean> => {
  ...
  if(error) {
    console.error(`Error checking if file exists in bucket '${bucketName}': `, error);
    return false;
  }
...
export const getPublicUrl = async (bucketName: string, filePath: string): Promise<string | null> => {
